...
import { getRolesAutorizadoresCombobox } from 'services/nivelDiscrecionalidadService';
import useRemoteSearchSelect from 'hooks/useRemoteSearchSelect';

const MIN_SEARCH_LENGTH = 2;

const RolAutorizadorSearchSelect = ({
  onSelect,
  selectedId,
  initialLabel = '',
  disabled = false,
  label = 'Rol autorizador',
  placeholder = 'Buscar rol autorizador',
}) => {
  const searchRoles = useCallback(
    (searchTerm = '') => getRolesAutorizadoresCombobox(1, searchTerm),
    []
  );
          ...
          <ul className="absolute z-50 top-full left-0 w-full bg-white border border-slate-200 rounded-lg mt-1 max-h-60 overflow-y-auto shadow-xl">
            {suggestions.length > 0 ? (
              suggestions.map((item) => (
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400">Rol habilitado para aprobar o rechazar evaluaciones</p>
                  </div>
                </li>
              ))
            ) : searchError ? (
              ...
              <li className="px-4 py-3 text-slate-500 text-xs italic text-center">
                No se encontraron roles autorizadores.
              </li>
            )}
          </ul>
        )}
...
export default RolAutorizadorSearchSelect;
