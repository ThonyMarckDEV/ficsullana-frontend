...
const AgregarAsesor = () => {
  ...
  const renderFormStep = () => {
    switch (currentStep) {
      ...
      case 2: return <CuentaForm data={formData.asesor} handleChange={(e) => handleChange(e, 'asesor')} />;
      default: return null;
    }
  };
        ...
        <form className="bg-white p-8 rounded-xl shadow-xl border border-slate-100 min-h-[400px] flex flex-col justify-between">
          ...
          <div className="flex justify-between mt-10 pt-6 border-t border-slate-100">
            ...
            {currentStep < STEPS.length ? (
              ...
              <button type="button" onClick={handleSubmit} disabled={loading} className="px-8 py-3 text-fic-dark bg-fic-yellow rounded-lg hover:bg-yellow-400 font-black uppercase shadow-lg">{loading ? 'Guardando...' : 'Crear Asesor'}</button>
            )}
          </div>
        </form>
      </div>
