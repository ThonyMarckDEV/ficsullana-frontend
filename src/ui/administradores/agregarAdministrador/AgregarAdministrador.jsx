...
import { createAdministrador } from 'services/administradorService';
...
const initialFormData = {
  ...
  administrador: { username: '', password: '', password_confirmation: '' }
};
...
const AgregarAdministrador = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await createAdministrador(formData);
      setAlert({ type: 'success', message: response.message || 'Administrador registrado correctamente.' });
      setTimeout(() => navigate('/personal/listar-administradores'), 1500);
    } catch (error) {
      setAlert(handleApiError(error, 'Error al registrar el administrador'));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="container mx-auto p-6 min-h-screen">
      <PageHeader 
        ...
        subtitle="Registro de personal con privilegios de gestión"
        icon={UserPlusIcon}
        ...
        <div className="flex items-center w-full relative">
            ...
            {STEPS.map((step) => (
                <div key={step.id} className="flex-1 flex flex-col items-center relative">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center font-black text-lg border-4 transition-all duration-300 z-10 ${currentStep >= step.id ? 'bg-fic-red border-white text-white shadow-lg scale-110' : 'bg-white border-slate-300 text-slate-400'}`}>
                        {currentStep > step.id ? <CheckIcon className="w-6 h-6"/> : step.id}
                    </div>
                    <span className={`mt-2 text-xs font-bold uppercase ${currentStep >= step.id ? 'text-fic-red' : 'text-slate-400'}`}>{step.name}</span>
                </div>
            ))}
        </div>
      </div>
        ...
        <form className="bg-white p-8 rounded-xl shadow-xl border border-slate-100 min-h-[400px] flex flex-col justify-between">
          <div>
            {currentStep === 1 ? (
                ...
                <CuentaForm data={formData.administrador} handleChange={(e) => handleChange(e, 'administrador')} />
            )}
          </div>
          <div className="flex justify-between mt-10 pt-6 border-t border-slate-100">
            ...
            {currentStep < STEPS.length ? (
              ...
              <button type="button" onClick={handleSubmit} disabled={loading} className="px-8 py-3 text-fic-dark bg-fic-yellow rounded-lg hover:bg-yellow-400 font-black uppercase shadow-lg">{loading ? 'Guardando...' : 'Crear Administrador'}</button>
            )}
          </div>
        </form>
      </div>
